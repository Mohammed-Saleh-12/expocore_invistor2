...
**الاستجابة المتوقعة (`data`):** `ExhibitionModel` كامل يشمل `map_data` و`sponsor_events` — راجع [ExhibitionModel](#exhibitionmodel).
...
| **الكنترولر** | `ExhibitionDetailController` ← عند الدخول إلى صفحة تفاصيل المعرض لجلب أجنحته |
...
> **ملاحظة:** `services` عبارة عن `Map<String, bool>` ديناميكي — المفاتيح تأتي من `BoothModel.services` (Map<String, double>) ويُرسَل كل مفتاح بقيمة `true/false` حسب اختيار المستخدم. لا يوجد حقول ثابتة للخدمات.
...
| `cover_image` | `File?` | صورة غلاف الجناح |
...
> **ملاحظة:** يُرسَل `start_date` و`end_date` بدلاً من `date` و`duration_days`. إذا كانت الفعالية يوماً واحداً تكون `end_date == start_date`.
...
| الحقل | النوع | الوصف |
|---|---|---|
| `avatar` | `File` | ملف الصورة الشخصية |
...
```
> يُحدَّث `last_message` و`last_time` في وثيقة المحادثة ضمن نفس الـ batch.
...
**الحقول المقروءة:**
```
id (doc ID → int), visitor_name, visitor_initials, color (hex string), unread_count, last_message, last_time, messages[]
```
...
### 14.3 تعليم رسائل محادثة زائر كمقروءة
| الخاصية | القيمة |
|---|---|
| **الملف** | `VisitorMessagesFirebaseData.markConversationRead()` |
| **الكنترولر** | `VisitorMessagesController` ← عند فتح المحادثة |

> يُعيِّن `is_read = true` على كل رسالة غير مقروءة في الـ sub-collection عبر batch write.
...
| — | **حقول بيانات الشركة** (تُملأ حين يكون الجناح محجوزاً) | | | |
| 14 | `companyName` | `String?` | `company_name` | اسم الشركة الحاجزة |
| 15 | `companyEmail` | `String?` | `company_email` | بريد الشركة الحاجزة |
| 16 | `companyInitials` | `String?` | `company_initials` | الأحرف الأولى للشركة |
...
| 20 | `durationDays` | `int` | `duration_days` | المدة بالأيام (محسوبة من الرد) |
...
| 25 | `bookedServices` | `List<String>` | `booked_services` | أسماء الخدمات التي تم اختيارها |
| 26 | `notes` | `String` | `notes` | ملاحظات الحجز |

> **ملاحظة مهمة:** لا توجد حقول ثابتة للخدمات (`screenService` / `setupService` / ...) — الخدمات ديناميكية بالكامل عبر حقل `services: Map<String, double>`.
...
| 5 | `services` | `List<String>` | `services` | خدمات المعرض المتاحة |
...
**Getters محسوبة:**
- `imageUrl` → أول عنصر في `images` أو `''` — للتوافق مع الكود القديم
...
| 6 | `date` | `String` | `date` | تاريخ بدء الفعالية (للعرض) |
| 7 | `time` | `String` | `time` | |
...
| 15 | `durationDays` | `int` | `duration_days` | افتراضي: 1 — من الرد فقط، لا يُرسَل في الإنشاء |
...
> **ملاحظة إنشاء الفعالية:** عند إرسال طلب الإنشاء يُرسَل `start_date` و`end_date` (لا `date` ولا `duration_days`). الحقل `durationDays` في الموديل يُملأ من رد الـ API فقط.
...
**`toJson()` يُرسَل عند الإنشاء (4 حقول فقط):** `event_id, selected_duration_label, selected_days, price`  
...
| 7 | `status` | `String` (mutable) | `status` | `pending` \| `approved` \| `rejected` — قابل للتعديل مباشرةً |
| 8 | `qrCodeData` | `String?` (mutable) | `qr_code_data` | nullable — قابل للتعديل مباشرةً |
| 9 | `ticketNumber` | `String?` (mutable) | `ticket_number` | nullable — قابل للتعديل مباشرةً |
...
| 5 | `halls` | `List<MapHallModel>` | `halls` | قائمة القاعات |
