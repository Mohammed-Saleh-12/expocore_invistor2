  ...
  Widget build(BuildContext context) {
    switch (request.type) {
      ...
      case WebDetailType.booth:
        return _BoothDetail(b: request.data as BoothModel, report: request.extra as ReportModel?);
      ...
      case WebDetailType.sponsorship:
        return WebSponsorshipDetailPage(booking: request.data as SponsorshipBookingModel);
                      ...
                      ClipRRect(
                        ...
                        child: Image.network(
                          ...
                          errorBuilder: (_, __, ___) => Container(
                            height: 260, color: AppColors.darkSurface,
                            child: Icon(Icons.image, color: AppColors.grey, size: 56),
                          ),
                        ),
                      ),
                    Padding(
                        ...
                        children: [
                          Row(
                            children: [
                              Expanded(
                                child: Text(title,
                                    style: TextStyle(color: WebTheme.text, fontSize: 26, fontWeight: FontWeight.w900)),
                              ),
                              if (badge != null) badge!,
                            ],
                          ),
...
Widget _infoRow(IconData icon, String label, String value) => Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(value,
                style: TextStyle(color: WebTheme.text, fontSize: 14, fontWeight: FontWeight.w600)),
          ),
        ],
      ),
    );

Widget _desc(String t) =>
    Text(t, style: TextStyle(color: AppColors.grey.withOpacity(0.9), fontSize: 14, height: 1.8));

Widget _chips(String label, List<String> items) => Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: TextStyle(color: AppColors.grey, fontSize: 13, fontWeight: FontWeight.w600)),
        ...
        Wrap(
          ...
          children: items.map((s) => Container(
            ...
            child: Text(s, style: TextStyle(color: AppColors.darkPink, fontSize: 12)),
          )).toList(),
        ),
      ],
    );

Widget _actionBtn(String label, {required bool filled, required VoidCallback onTap}) => GestureDetector(
      onTap: onTap,
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 26, vertical: 14),
        decoration: BoxDecoration(
          gradient: filled ? AppColors.favoriteGradient : null,
          border: filled ? null : Border.all(color: AppColors.darkPrimary.withOpacity(0.5)),
          borderRadius: BorderRadius.circular(12),
        ),
        child: Text(label,
            style: TextStyle(
  ...
  Widget build(BuildContext context) {
    ...
    return _DetailScaffold(
      ...
      children: [
        ...
        _infoRow(Icons.location_on_outlined, 'الموقع', '${e.location}، ${e.city}'),
        _infoRow(Icons.calendar_today_outlined, 'التاريخ', '${e.startDate} — ${e.endDate}'),
        _infoRow(Icons.grid_view_rounded, 'الأجنحة المتاحة', '${e.availableBooths} جناح'),
        ...
        ..._services.map((s) => Padding(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 12),
                Text(s.$2, style: TextStyle(color: WebTheme.text, fontSize: 14)),
              ]),
            )),
        const SizedBox(height: 24),
        ...
        Obx(() {
          final list = events.exhibitionSponsorEvents
              ...
              .toList();
          if (list.isEmpty) {
            return Text('لا توجد فعاليات إعلانية لهذا المعرض حالياً',
                style: TextStyle(color: AppColors.grey.withOpacity(0.8), fontSize: 13));
          }
          return Column(children: list.map((ev) => _SponsorEventRow(event: ev, ctrl: events)).toList());
        }),
      ],
      actions: [
        // مفضلة
        Obx(() {
          ...
          return _actionBtn(isFav ? '★ في المفضلة' : '☆ أضف للمفضلة', filled: false,
              onTap: () => fav.toggleFavoriteExhibition(e));
        }),
        // تواصل
        _actionBtn('تواصل مع الإدارة', filled: false, onTap: () {
          Get.find<MessagesController>().prepareConversationForExhibition(e.name);
          WebNavController.to.select(6);
        }),
        // استكشاف الأجنحة
        _actionBtn('استكشاف الأجنحة', filled: true, onTap: () => WebNavController.to.select(2)),
      ],
    );
  }
}
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(event.name, style: TextStyle(color: WebTheme.text, fontSize: 14, fontWeight: FontWeight.w700)),
  ...
  Widget build(BuildContext context) => _DetailScaffold(
        ...
        actions: [
          if (b.status == 'active' && report != null)
            _actionBtn('عرض التقرير', filled: false, onTap: () => WebNavController.to.openReport(report!)),
          if (b.status == 'active' && report != null) const SizedBox(width: 12),
          _actionBtn('حفظ', filled: true, onTap: () {
            Get.snackbar('تم', 'تم تحديث بيانات الجناح', snackPosition: SnackPosition.BOTTOM);
          }),
        ],
      );
}
  ...
  Widget build(BuildContext context) => _DetailScaffold(
        ...
        children: [
          Container(
            ...
            child: Text(e.type, style: TextStyle(color: AppColors.darkPink, fontSize: 13, fontWeight: FontWeight.w600)),
          ),
          const SizedBox(height: 16),
          _desc(e.description),
          const SizedBox(height: 18),
          _infoRow(Icons.calendar_today_outlined, 'التاريخ', '${e.date} • ${e.time}'),
          _infoRow(Icons.store_outlined, 'الجناح', e.boothNumber.isEmpty ? '—' : e.boothNumber),
          _infoRow(Icons.people_outline, 'المسجّلون', '${e.registeredCount} / ${e.maxParticipants}'),
          if (e.hasBookableSeats)
            _infoRow(Icons.event_seat_outlined, 'المقاعد', '${e.bookedSeats} / ${e.totalSeats}'),
          if (e.ticketPrice > 0)
            _infoRow(Icons.payments_outlined, 'سعر التذكرة', '${e.ticketPrice.toInt()} ر.س'),
        ],
        actions: [
          _actionBtn('طلبات التذاكر', filled: true, onTap: () => WebNavController.to.openTicketRequests(e)),
        ],
      );
}
    ...
    canvas.drawPath(
      ...
      Paint()
        ..shader = LinearGradient(
          ...
          colors: [AppColors.darkPrimary.withOpacity(0.4), AppColors.darkPrimary.withOpacity(0.0)],
