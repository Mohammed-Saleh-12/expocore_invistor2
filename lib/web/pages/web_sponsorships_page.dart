  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          const WebSectionHeader(title: 'رعاياتي', subtitle: 'الفعاليات التي ترعاها في المعارض'),
            ...
            if (list.isEmpty) {
              return Container(
                ...
                child: Column(children: [
                  Icon(Icons.workspace_premium_outlined, size: 56, color: AppColors.grey.withOpacity(0.5)),
                  const SizedBox(height: 12),
                  Text('لا توجد رعايات حتى الآن', style: TextStyle(color: AppColors.grey)),
                  const SizedBox(height: 6),
                  Text('تصفّح فعاليات المعارض وقدّم طلب رعاية', style: TextStyle(color: AppColors.grey, fontSize: 12)),
                ]),
              );
            }
            return Column(
              children: list.map((b) => _SponsorshipCard(booking: b)).toList(),
            );
          }),
        ],
      ),
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Container(
              width: 50, height: 50,
              decoration: BoxDecoration(gradient: AppColors.favoriteGradient, borderRadius: BorderRadius.circular(13)),
              child: Icon(Icons.workspace_premium_rounded, color: WebTheme.text, size: 24),
            ),
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(booking.eventName, maxLines: 1, overflow: TextOverflow.ellipsis,
                      style: TextStyle(color: WebTheme.text, fontSize: 16, fontWeight: FontWeight.w800)),
                  const SizedBox(height: 4),
                  Text(booking.exhibitionName, maxLines: 1, overflow: TextOverflow.ellipsis,
                      style: TextStyle(color: AppColors.grey, fontSize: 12)),
                  const SizedBox(height: 8),
                  Row(children: [
                    _meta(Icons.timelapse_rounded, booking.selectedDurationLabel),
            ...
            Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text('${booking.price.toInt()} ر.س',
                    style: TextStyle(color: AppColors.darkAccent, fontSize: 16, fontWeight: FontWeight.w800)),
              ],
            ),
          ],
        ),
