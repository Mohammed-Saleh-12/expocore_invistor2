...
class ExhibitionEventsView extends GetView<EventsController> {
        ...
        return ListView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            return SponsorEventCard(
              event: ev,
              onTap: () => _showSponsorshipBottomSheet(context, ev),
  ...
  void _showSponsorshipBottomSheet(
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _SponsorshipBottomSheet(event: event),
    );
  }
}

class _SponsorshipBottomSheet extends StatefulWidget {
  final ExhibitionSponsorEvent event;
  const _SponsorshipBottomSheet({required this.event});

  @override
  State<_SponsorshipBottomSheet> createState() =>
      _SponsorshipBottomSheetState();
}

class _SponsorshipBottomSheetState extends State<_SponsorshipBottomSheet> {
  ...
  Widget _stepDetails(bool isDark) => Column(
        ...
        children: [
          ...
          _detailRow(Icons.store_outlined, 'المعرض', widget.event.exhibitionName),
          _detailRow(Icons.calendar_today_outlined, 'التاريخ', widget.event.date),
          _detailRow(Icons.location_on_outlined, 'المكان', widget.event.place),
          ...
          Text('خيارات المشاركة الإعلانية',
              style: const TextStyle(
                  fontSize: 14, fontWeight: FontWeight.w700)),
  ...
  Widget _stepBookingForm(bool isDark) => Column(
        ...
        children: [
          ...
          _autoFilledField(isDark, 'اسم الشركة', ctrl.companyNameCtrl.text,
              Icons.business_outlined),
          const SizedBox(height: 10),
          _autoFilledField(isDark, 'الموقع الإلكتروني', ctrl.companyWebCtrl.text,
              Icons.language_outlined),
          const SizedBox(height: 10),
          _autoFilledField(isDark, 'رقم التواصل', ctrl.companyPhoneCtrl.text,
            ...
            Expanded(
              ...
              child: Obx(() => ElevatedButton(
                    ...
                    child: ctrl.isBooking.value
                        ...
                        : Text(
                            _step == 0 ? 'التالي: بيانات الشركة' : 'إرسال طلب الرعاية',
                            style: const TextStyle(
                                color: Colors.white,
                                fontWeight: FontWeight.w700)),
                  )),
  ...
  Widget _uploadBox(bool isDark, IconData icon, String label) =>
      GestureDetector(
        ...
        child: Container(
          ...
          child: Row(
            children: [
              Icon(icon, color: AppColors.darkPrimary.withOpacity(0.7),
                  size: 22),
              const SizedBox(width: 12),
              Text(label,
                  style: const TextStyle(
