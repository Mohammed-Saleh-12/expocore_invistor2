...
class MySponshorshipsView extends GetView<EventsController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(title: 'فعالياتي الإعلانية'),
      body: Obx(() {
        ...
        return ListView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            return _SponsorshipBookingCard(
              booking: booking,
              onTap: () =>
                  Get.toNamed(AppRoutes.SPONSORSHIP_DETAIL, arguments: booking),
            );
          },
        );
      }),
...
class _SponsorshipBookingCard extends StatelessWidget {
  ...
  const _SponsorshipBookingCard({required this.booking, required this.onTap});
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _infoRow(Icons.date_range_outlined, '${booking.startDate} إلى ${booking.endDate}'),
            const SizedBox(height: 4),
            _infoRow(Icons.location_on_outlined, booking.place),
            const SizedBox(height: 4),
            _infoRow(Icons.groups_outlined, 'السعة ${booking.capacity} • ${booking.registeredCount} مسجل'),
            const SizedBox(height: 4),
            _infoRow(Icons.schedule_outlined, booking.selectedDurationLabel),
            const SizedBox(height: 12),
            Container(
