...
class ReportsListView extends GetView<ReportsController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'التقارير والتحليلات', showBack: false, actions: [
        IconButton(icon: const Icon(Icons.filter_list, color: AppColors.darkPrimary), onPressed: () {}),
      ]),
      body: Column(children: [
        Obx(() => SingleChildScrollView(
          scrollDirection: Axis.horizontal,
          padding: const EdgeInsets.fromLTRB(16, 10, 16, 0),
          child: Row(children: controller.typeFilters.map((f) {
        ...
        Expanded(child: Obx(() {
          if (controller.filtered.isEmpty) return const EmptyWidget(message: 'لا توجد تقارير');
          return ListView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              return Obx(() => ReportCard(
                ...
                onView: () => Get.toNamed(AppRoutes.REPORT_DETAIL, arguments: r),
  ...
  Widget _dateRangeBar(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(children: [
        const Icon(Icons.date_range_outlined, size: 18, color: AppColors.grey),
        const SizedBox(width: 8),
        const Text('من: 2026-07-01  إلى: 2026-07-31', style: TextStyle(fontSize: 12, color: AppColors.grey)),
        const Spacer(),
        GestureDetector(
          onTap: () {},
          child: const Text('تغيير', style: TextStyle(fontSize: 12, color: AppColors.darkPrimary, fontWeight: FontWeight.w600)),
        ),
      ]),
    );
  }
