            ...
            Expanded(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  return Stack(
                    children: [
                      _MapCanvas(ctrl: ctrl, mapModel: mapModel, isDark: isDark),
                      Obx(() {
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text(mapModel.exhibitionName,
                    style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: isDark ? Colors.white : AppColors.lightPrimary)),
              ),
              IconButton(
...
class _LegendRow extends StatelessWidget {
  ...
  Widget _item(Color c, String label, bool isDark) => Row(
        children: [
          ...
          Text(label, style: TextStyle(fontSize: 10, color: isDark ? Colors.white70 : AppColors.grey)),
        ],
      );
}
...
class _BoothCompanyDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return TweenAnimationBuilder<double>(
      ...
      child: _DialogCard(booth: booth, company: company, accentColor: accentColor, isDark: isDark, onClose: ctrl.clearSelection),
    );
  }
}
...
class _DialogCard extends StatelessWidget {
  ...
  const _DialogCard({required this.booth, required this.company, required this.accentColor, required this.isDark, required this.onClose});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: accentColor.withOpacity(0.55), blurRadius: 22, spreadRadius: 2),
          BoxShadow(color: accentColor.withOpacity(0.22), blurRadius: 50, spreadRadius: 8),
  ...
  Widget _buildHeader(String name, String email, String initials, Color accent, bool isDark) {
    return Container(
      padding: const EdgeInsets.fromLTRB(14, 14, 12, 12),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [accent.withOpacity(0.18), accent.withOpacity(0.04)], begin: Alignment.topRight, end: Alignment.bottomLeft),
      ...
      child: Row(
        children: [
          Container(
            width: 46, height: 46,
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [accent, accent.withOpacity(0.65)], begin: Alignment.topLeft, end: Alignment.bottomRight),
              shape: BoxShape.circle,
              boxShadow: [BoxShadow(color: accent.withOpacity(0.5), blurRadius: 12, offset: const Offset(0, 4))],
            ),
            child: Center(child: Text(initials, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900, letterSpacing: 0.5))),
          ),
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(name, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: isDark ? Colors.white : Colors.black87)),
                const SizedBox(height: 3),
                Row(
                  children: [
                    Icon(Icons.email_outlined, size: 11, color: accent),
                    const SizedBox(width: 4),
                    Flexible(child: Text(email, style: TextStyle(fontSize: 11, color: isDark ? Colors.white60 : AppColors.grey), overflow: TextOverflow.ellipsis)),
                  ],
                ),
              ],
            ),
    ...
    return AnimatedContainer(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Container(
                ...
                child: Center(child: Icon(Icons.store_mall_directory_rounded, color: hall?.color ?? AppColors.darkPrimary, size: 22)),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('الجناح ${booth.number}', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: isDark ? Colors.white : Colors.black87)),
          ...
          Row(
            children: [
              _Chip(icon: Icons.straighten_rounded, label: '${booth.area.toInt()} م²', isDark: isDark),
              const SizedBox(width: 8),
              _Chip(icon: Icons.height_rounded, label: 'ارتفاع ${booth.height}م', isDark: isDark),
              const SizedBox(width: 8),
              _Chip(icon: Icons.grid_3x3_rounded, label: '${booth.gridWidth}×${booth.gridDepth}', isDark: isDark),
            ],
          ),
          if (booth.amenities.isNotEmpty) ...[
            const SizedBox(height: 10),
            Wrap(spacing: 6, children: booth.amenities.map((a) => _AmenityTag(label: a, isDark: isDark)).toList()),
          ],
          const SizedBox(height: 16),
          Row(
            children: [
