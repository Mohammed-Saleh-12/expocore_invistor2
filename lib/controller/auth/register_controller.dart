...
class RegisterController extends GetxController {
  ...
  Future<void> register() async {
    ...
    if (result['status'] == true) {
      ...
      Get.snackbar('تم التسجيل', 'تم إنشاء حسابك بنجاح. يرجى تسجيل الدخول.',
          snackPosition: SnackPosition.BOTTOM);
    } else {
      status.value = StatusRequest.failure;
      Get.snackbar('خطأ في التسجيل', result['message'] ?? 'حدث خطأ',
          snackPosition: SnackPosition.BOTTOM);
    }
  }
  ...
  void onClose() {
    ...
    phoneCtrl.dispose(); websiteCtrl.dispose(); passCtrl.dispose();
    confirmCtrl.dispose();
    super.onClose();
  }
}
